...
var menuTab = function menuTab() {
  ...
  let array  = Array.prototype.slice.call(button);
  array.forEach( btn => {
    btn.addEventListener('click', event => {
      // get parent
      let parent = this.documentElement.parentNode
      let allbtn = parent.querySelectorAll('.tab-btn');
      let btnarr = Array.prototype.slice.call(allbtn);
      let target = btn.dataset.target
      let alltgt = parent.querySelectorAll('.tab-content');
      let tgtarr = Array.prototype.slice.call(alltgt);
      // button
      btnarr.forEach( childbtn => {
        childbtn.classList.remove('active')
      });
      btn.classList.add('active')
      // target element
      tgtarr.forEach( targetelm => {
